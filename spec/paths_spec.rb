...
describe HttpParser::Paths do
  ...
  it "root dir should be correct" do
    HttpParser.root_dir.should == @root_dir
  end

  it "home dir should be correct" do
    HttpParser.home_dir.should == @root_dir
  end

  %w[ bin lib spec config data log tmp ].each do |d|
    it "#{d} path should be correct" do
      HttpParser.send( "#{d}_path" ).should == File.join(@root_dir, "#{d}/" )
    end
  end
